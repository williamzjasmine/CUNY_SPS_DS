...
Based our initial definition of $f_U(u)$, the left integral must be 0, and since we know $w \leq1 $, the second integral must be equal to $u$. Thus, we have: 
...
Similarly, if If $0 \leq w \leq 1$, then we know that $w - 1$ must be less than 1. Following the same pattern we have: 
...
# use 30,003 simulations to match seq values in next cell
sims <- 30003
Ws <- numeric(sims)
...
```{r}
range1x <- seq(0, 1, 0.0001)
range1y <- (range1x * range1x) / 2
range2x <- seq(1, 2, 0.0001)
