...
Next, we can find the variance of the Gamma probability distribution using the fact that $\sigma^2 = E(X^2) - E(X)^2$. However, we must first find $E(X^2)$, which, given a probability density function $f(x)$ is defined as: $E(X^2) = \int f(x) \cdot x^2$:
...
\end{equation}
$$
The steps of solving this integral are quite similar to the ones shown above, and are shown below:
...
Thus, we can finally compute the variance $\sigma^2$ as follows:
...
Using the values of $n$ and $\lambda$ chosen in part 1A, we get that $E(X) = \frac{n}{\lambda} = 10/5 = 2$ and $\sigma^2 = \frac{n}{\lambda^2} = \frac{10}{25}=0.4$. These are very close to the empirically calculated values for the $X$ probability distribution in part 1A.
...
We see indeed that the that mean of the $Z$ distribution from part 1A is very close to $\frac{1}{\lambda} = \frac{1}{5} = 0.2$.

The distribution $Y$ from part 1A is just a sum of $n$ exponential random variables: $Y = \sum_1^n Z$. Because the expected value of the sum of several random variables is equal to the sum of their expectations (i.e. $E[U+W] = E[W] + E[U]$), the expected value of $Y$ (a distribution that represents the sum of $n$ exponential random variables) can be written as:
...
In this case, $P(B|A) = P(Z > \lambda/2 \ |\  Z > \lambda) = 1$, since if $Z$ is greater than $\lambda$ it must be greater than $\lambda / 2$. Thus, we can easily compute $P$ in the cell below:
...
We can determine the probability by mimicking the steps used in part 1C. Once again we can use Baye's rule with the helpful addition that $P(B|A) = P(Z > \lambda \ |\  Z > 2\lambda) = 1$, since if $Z$ is greater than $2\lambda$ it must be greater than $\lambda$. $P$ is computed in the cell below:
...
Thus, $P(Z > \lambda \ | \  Z > \lambda/2) \approx 0.167$. 

Following in the same footsteps as in part 1C, we can show adherence of the memoryless property for this probability by proving that:
...
We have already determined $P(Z > \lambda)$ in part 1C, and so can immediately compute $\frac{P(Z > 2\lambda)}{P(Z > \lambda)}$:
...
We can determine the probability by mimicking the steps used in parts 1C and 1D. Once again we can use Baye's rule with the helpful addition that $P(B|A) = P(Z > \lambda \ |\  Z > 3\lambda) = 1$, since if $Z$ is greater than $3\lambda$ it must be greater than $\lambda$. $P$ is computed in the cell below:
...
Thus, $P(Z > \lambda \ | \  Z > \lambda/2) \approx 0$. Given the choices of $n$ and $\lambda$, it appears there were no samples that satisfied this conditional probability. Using R we can compute the theoretical value of this probability, which confirms that it is very close to 0:
...
row_sum <- apply(YZ_quants, 1, sum)
col_sum <- sum(apply(YZ_quants, 2, sum))
YZ_quants / col_sum
```
...
Y_quants <- quantile(a, probs=c(0.25, 0.5, 0.75, 1))
Z_quants <- quantile(b, probs=c(0.25, 0.5, 0.75, 1))
YZ_quants <- t(tcrossprod(Y_quants, Z_quants))
row_sum <- apply(YZ_quants, 1, sum)
col_sum <- sum(apply(YZ_quants, 2, sum))
...
Z <- Y + sample(1:6, 10000, replace=TRUE)
freqs <- unname(table(Y, Z))
row_sums <- apply(freqs, 1, sum)
...
fisher.test(freqs, simulate.p.value = TRUE)
```
