---
title: "Data 605 - Final Project - Part 2"
...
This part of the assignment involves using data from Kaggle's [Housing Prices: Advanced Regression Techniques competition](https://www.kaggle.com/c/house-prices-advanced-regression-techniques). The data from this page is loaded in the cell below:
...
We see right away from the table above that the variable `Utilities` is the same for all houses except one, and as such will end up having little to no predictive power. As such, it is removed in the cell below:
...
We see that most of the distributions maintain a certain level of normality, with the exception of `YrSold` and `YearBuilt`. In the `YrBuilt` distribution, we can clearly see the presence of the different housing booms experienced by the United States. 

The below creates scatterplots of our numeric fields with the target variable, `SalePrice`: 
...
Overall, we can see positive correlation with all the chosen explanatory variables with the exception of maybe `YrSold` and `OverallCond`, indicating that most of these variables may prove useful when actually building our model. 

We can check the correlation between the independent by producing a correlation matrix: 
...
The diagonal of the precision matrix provide variance inflation factors (VIF), which provide us with another way to analyze the multicollinearity between the explanatory variables:
...
We can see that there is some level of collinearity among all of our explanatory variables (once again, with the exception of `YrSold`). This is expected, given the high level of correlation that the other variables had with one another. However, VIF is typically a better indicator of collinearity than correlation, and generally it is accepted that predictor variables with VIF values less than 5 are acceptable to use in a linear model. 
...
As such, it could be appropriate to attempt to fit this the distribution of `SalePrice` to an exponential probability density function (PDF). The exponential distribution, given by the formula below:
...
only has one parameter $\lamba$ that is required for the fit. The best-fit value of $\lambda$ is determined below using the `fitdistr` function: 
...
Comparing the two the theoretical and empirical yields a solid agreement, which would only improve given a larger number of samples. 
...
Before building a linear regression model, the cell below converts the two remaining categorical variables `Neighborhood` and `BldgType` into dummy variables, in the hopes unhelpful categories from each can be removed from the model if they are not effective predictors. 
...
The $R^2$ value remains unchanged despite having reduced the number of features in our model by 17, giving further evidence to the fact that they were insignificant predictors. 
...
The above sets up the testing data in the same way as the test data, and then uses it to create predictions, which are stored in `preds`. The cell below then creates a dataframe of these predictions that can be used to create a `.csv` file that can be uploaded to Kaggle. 
...
The cell below rebuilds our linear model with after first removing these outlier observations from the training set: 
