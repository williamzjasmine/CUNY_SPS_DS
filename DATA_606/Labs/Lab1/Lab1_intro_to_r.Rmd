...
WJ Response: 
Using the plot created from the ggplot command we just learned, we see that the number of girls baptized over time increases (from 4,683 in 1629 to 7,288 in 1710), which seems consistent with typical population growth trends. However, there is a significant drop in the total girl baptisms from ~ 1640-1665, and another in the year 1704. These dips could be an artifact of certain historical events or simply just errors in reporting.  
...
ggplot(data = arbuthnot, aes(x = year, y = boy_ratio)) + 
  geom_line()
```
Looking at the plot created by the code above, we can see that the proportion of boys baptized over time flucuates around the 52% mark, in what looks like a random walk. The average ratio seems to dip a little lower in the right portion of the plot (after the year 1670), but there is not a single year in which this ratio dips below 50%, which would likely be the hypothesized or expected value in this case. 
...
glimpse(present)
```
The glimpse command above shows that the "present" data frame contains three columns: year, boys, and girls (same as the arbuthnot data  frame).

```{r}
present %>%
  summarize(min = min(year), max = max(year))
```
The summarize function below shows that the data in the "present" dataframe spans from 1940 to 2002. 
...
```
Using the code above we see that the average total births from the "present" data frame is over 320 times greater than the averave total births from the "arbuthnot" data frame. This makes sense given that the data from the "present" data frame is taken about 300 years later when birth rates were much higher.


3.  Make a plot that displays the proportion of boys born over time. What do you see? 
...
ggplot(data = present, aes(x = year, y = boy_ratio)) + 
  geom_line()
```
Looking at the plot created from the above code, we see that the conclusions drawn from  Arbuthnot's research do hold up: there is not a single year between 1940 and 2002 in which the proportion of boys being born dips below 50%. Over time however, it does appear that this proportion is dropping.
...
present %>%
  arrange(desc(total))
```
The sample code provided above actually can be used directly, since I already created a "total" column in the "present" dataset. The output of this code shows that the year with the highest number of total  births is 1961. 
